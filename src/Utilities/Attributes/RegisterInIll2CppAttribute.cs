...
/// This class was ported from Reactor 2.1 and is allowed to be modified under LGPL 3.0
///
///
/// All credit goes to the people with Reactor (js6pak, AlexejheroYTB) for this class.
/// If there were more Reactor things I needed Reactor for I would just it as a dependency, but this is the only
/// class I needed so it seems a bit redundant to link the whole project.
///
/// Once again all credit for this class goes to js6pak, AlexejheroYTB, and the rest of Reactor.
...
public sealed class RegisterInIl2CppAttribute : Attribute
{
    ...
    private static void RegisterType(Type type, Type[] interfaces)
    {
        ...
        if (baseTypeAttribute != null) RegisterType(type.BaseType!, baseTypeAttribute.Interfaces);

        if (ClassInjector.IsTypeRegisteredInIl2Cpp(type)) return;
        ...
        {
            VentLogger.Error($"Failed to register {type.FullDescription()}: {e}");
        }
    }
